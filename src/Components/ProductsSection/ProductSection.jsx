        ...
        {productData.map((product) => (
          <div key={product._id} className=" relative rounded-xl overflow-hidden group">
            <div className="h-full w-full">
              <img className="h-full w-full object-cover group-hover:scale-105 transition-all ease-in-out" src={product?.productImage} alt="" />
            </div>
            <div className="absolute top-0 bg-[#282828]/[.50] w-full p-2 text-center backdrop-blur">
              <h1 className="text-xl font-bold text-white"> {product?.productName}</h1>
            </div>
          </div>
        ))}
      </div>
